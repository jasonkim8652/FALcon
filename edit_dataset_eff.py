...
def edit_initial_dataset(
	...
	condition = (df['LigEfficiency'] < 0.0)
	df_ = df[condition]
...
def edit_active_learning_dataet(
	...
	df_sampled = df_sampled[['ID','SMILES','SMILES_BB1','SMILES_BB2','LigEfficiency']]
	df_train = pd.concat([df_train, df_sampled])

	df_remain = df_remain[num_train:]
	df_remain = df_remain[['ID','SMILES','SMILES_BB1','SMILES_BB2','LigEfficiency']]
